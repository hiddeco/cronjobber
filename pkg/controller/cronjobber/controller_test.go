...
func cronJob() cronjobberv1.TZCronJob {
	return cronjobberv1.TZCronJob{
		ObjectMeta: metav1.ObjectMeta{
			Name:              "mycronjob",
			Namespace:         "snazzycats",
			UID:               types.UID("1a2b3c"),
			SelfLink:          "/apis/cronjobber.hidde.co/v1alpha1/namespaces/snazzycats/tzcronjobs/mycronjob",
			CreationTimestamp: metav1.Time{Time: justBeforeTheHour()},
		},
		Spec: cronjobberv1.TZCronJobSpec{
			Schedule:          "* * * * ?",
			ConcurrencyPolicy: cronjobberv1.AllowConcurrent,
...
var (
	...
	noDead     int64                          = -12345
	A          cronjobberv1.ConcurrencyPolicy = cronjobberv1.AllowConcurrent
	f          cronjobberv1.ConcurrencyPolicy = cronjobberv1.ForbidConcurrent
	R          cronjobberv1.ConcurrencyPolicy = cronjobberv1.ReplaceConcurrent
	T                                         = true
	F                                         = false
)
...
func TestSyncOne_RunOrNot(t *testing.T) {
	...
	testCases := map[string]struct {
		// sj spec
		concurrencyPolicy cronjobberv1.ConcurrencyPolicy
		suspend           bool
		schedule          string
		deadline          int64
	...
	for name, tc := range testCases {
		...
		syncOne(&sj, js, tc.now, jc, sjc, recorder, logger)
		expectedCreates := 0
		if tc.expectCreate {
			expectedCreates = 1
		}
		...
		for i := range jc.Jobs {
			...
			if controllerRef == nil {
				...
				if got, want := controllerRef.Kind, "TZCronJob"; got != want {
...
func TestCleanupFinishedJobs_DeleteOrNot(t *testing.T) {
	...
	for name, tc := range testCases {
		...
		cleanupFinishedJobs(&sj, js, jc, sjc, recorder, logger)
...
func TestSyncOne_Status(t *testing.T) {
	...
	for name, tc := range testCases {
		...
		// Run the code
		syncOne(&sj, jobs, tc.now, jc, sjc, recorder, logger)
